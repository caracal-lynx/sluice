...
```yaml
# shared/rules.yaml
# Shared DQ rule library for Style Co.
...
dq:
  ...
  rules:
    - field: STYLE_NO
      checks:
        - { type: style-coStyleNo }       # Expands to three checks at load time.
...
```typescript
// clients/style-co/plugins/ukVatNumber.rule.ts
...
```typescript
// clients/acme-corp/plugins/ifsGlAccount.rule.ts
import type { RulePlugin, RuleViolation } from '@caracal-lynx/sluice';
...
export const rule: RulePlugin = {
  id: 'ifsCostCentre',
  description: 'Must be a valid Acme Corp IFS cost centre code',
...
```typescript
// clients/style-co/plugins/ukNiNumber.rule.ts
import type { RulePlugin, RuleViolation } from '@caracal-lynx/sluice';
...
```typescript
// clients/acme-corp/plugins/erpAccountFormat.transform.ts
...
```typescript
// clients/style-co/plugins/seasonFromDate.transform.ts
import type { TransformPlugin } from '@caracal-lynx/sluice';
...
```typescript
// clients/style-co/plugins/styleDescClean.transform.ts
...
```typescript
// clients/acme-corp/plugins/priceWithMargin.transform.ts
import type { TransformPlugin, CustomFieldMapping } from '@caracal-lynx/sluice';
...
Loaded rule plugins (12):
  ├─ style-coSeasonCode         Style Co custom season code validation
  ├─ bcInventoryLocation     Business Central inventory location rules
  └─ ...

Loaded transform plugins (8):
  ├─ style-coStyleTransform     Style Co style mapping (old → new format)
...
Multi-source merge allows pipelines to combine data from 2+ source systems
before transformation and loading. Common scenarios:

- Master data in one source (e.g., MSSQL) with enrichment from another (e.g., Excel)
- Multi-warehouse inventory: central database + branch supplements
- Multi-channel orders: e-commerce + POS + manual entry, merged by order ID
- ERP pre-flight: compare customer records across IFS and Business Central
- Data reconciliation: union or intersect rows to find discrepancies

### Merge strategies

| Strategy | Behavior | Use case |
|----------|----------|----------|
| **coalesce** (default) | First non-null value, respecting priority order | Filling gaps; enrichment data |
| **priority-override** | Highest priority source wins (ignore nulls in lower) | Trusted source takes all |
| **union** | All rows from all sources (deduplicate by key) | See everything; combine completeness |
| **intersect** | Only rows present in ALL sources | Find common records; reconciliation |

### Pipeline YAML syntax

```yaml
pipeline:
  name: style-co-customer-sync
  client: style-co
  version: "1.0.0"
  entity: customer
  description: |
    Merge customer master from ERP and enrichment data from Excel,
    coalesce nulls by priority, then sync to Business Central.

# ── Multi-source block (replaces single 'source') ───────────

sources:

  # Source 1 — ERP (priority 1 = checked first for coalesce)
  - id: erp-master
    priority: 1
    adapter: mssql
    connection: ${ERP_CONNECTION}
    query: |
      SELECT CustomerID, Name, Phone, Email, null as RegistrationDate
      FROM dbo.Customer
      WHERE Active = 1

  # Source 2 — Excel enrichment (priority 2 = checked second)
  - id: excel-supplement
    priority: 2
    adapter: xlsx
    file: ./data/customer-enrichment.xlsx
    sheet: 0
    rename:
      "ID": "CustomerID"           # Align key column
      "Registration Date": "RegistrationDate"

# ── Merge configuration ──────────────────────────────────

merge:
  key: CustomerID               # Column(s) to join on (required)
  strategy: coalesce            # Pick first non-null per priority (default: coalesce)
  onUnmatched:                  # Rows present in only one source
    include                      # include all; warn if found (default)
    # include                    # or: include silently
    # exclude                    # or: drop unmatched rows  
    # error                      # or: fail if any unmatched found
  fieldStrategies:              # [Optional] override strategy per field
    - field: Email
      source: erp-master        # Force this source (ignores priority)
    - field: Phone
      strategy: priority-override # Use strategy override for this field only

  # [Optional] Log rows with conflicting values (when multiple sources differ)
  conflictLog: ./logs/merge-conflicts.csv

# Remaining pipeline sections unchanged (dq, transform, target, run)
```
...
```yaml
merge:
  key: [warehouse_id, sku]      # Composite key (array of columns)
  strategy: union               # Include all warehouse×sku combinations
```

### Strategy examples

#### Example 1: Coalesce (enrichment)

```yaml
Sources:
  A (priority 1): {ID: 1, name: "John", email: null}
  B (priority 2): {ID: 1, name: null, email: "john@example.com"}

Strategy: coalesce
Result:   {ID: 1, name: "John", email: "john@example.com"}
          (first non-null value from priority order)
```

#### Example 2: Union (combining inventory)

```yaml
Sources:
  Warehouse A: {sku: "STYLE-001", qty: 50}
  Warehouse B: {sku: "STYLE-002", qty: 30}

Strategy: union
Result:   Both rows included (all SKUs × warehouses)
```

#### Example 3: Intersect (reconciliation)

```yaml
Sources:
  IFS:              {ID: 1, ID: 2, ID: 3}
  Business Central: {ID: 2, ID: 3, ID: 4}

Strategy: intersect
Result:   Only {ID: 2, ID: 3} (present in both systems)
```

### Configuration constraints & validation

**Required:**
- `key` — one or more columns (must exist in all sources)
- `sources` — array with 2+ entries, each with unique `id` and `priority`
...
```typescript
export const MergeFieldStrategySchema = z.object({
  field:    z.string(),
  strategy: z.enum(['coalesce', 'priority-override']).optional(),
  source:   z.string().optional(),
}).refine(
  (s) => s.strategy !== undefined || s.source !== undefined,
  { message: 'fieldStrategy must specify strategy or source' }
);
...
export const MergeSchema = z.object({
  ...
  incrementalSource: z.string().optional(),  // Phase 3: incremental by source
});
```

### DuckDB staging

Merge uses the staging store's temporary tables:

```sql
stg_<sourceId>_1, stg_<sourceId>_2, ...  -- Input source tables (filled by adapters)
stg_merge_joined                          -- FULL OUTER JOIN on keys
stg_merge_conflicts                       -- [Optional] conflicting values log
stg_merged                                -- Final merged output
```

### Runtime behavior

**Merge execution (MergeEngine → strategy → SQL builder):**

1. **Join phase**: FULL OUTER JOIN all sources on key columns
   - Creates a row for each unique key (matched and unmatched)
   - Columns from each source prefixed with source ID if ambiguous

2. **Deduplicate phase** (union/intersect only)
   - Union: Include all keys (outer join result)
   - Intersect: Filter to keys present in ALL sources

3. **Coalesce phase** (all strategies)
   - For each output column, select first non-null value (priority order)
   - Respect field overrides (source pinning or strategy override)
   - Whitespace-only strings treated as blank (trimmed before coalesce)

4. **Conflict detection** (optional)
   - If `conflictLog` specified, log rows where sources differ after coalesce
   - Severity: informational (doesn't affect merge result)

5. **Output**
   - Key columns appear first in output table
   - Remaining columns appear in source priority order
   - Rows: depends on strategy and onUnmatched setting
...
pipeline:
  name: inventory-sync
  entity: stock-on-hand

sources:
  - id: central-db
    priority: 1
    adapter: mssql
    query: "SELECT sku, warehouse, qty, updated FROM dbo.Inventory"

  - id: branch-file
    priority: 2
    adapter: xlsx
    file: ./branch-inventory.xlsx

merge:
  key: [sku, warehouse]
  strategy: union
  onUnmatched: include
  conflictLog: ./logs/inventory-conflicts.csv
...
dq:
  rules:
    - field: sku
      checks:
        - { type: notNull, severity: critical }
        - { type: pattern, value: "^[A-Z0-9]{6}$", severity: critical }
...
```
Central DB (100 rows)  ─┐
                        ├──> Merge (union)  ──> 145 rows ──> DQ ──> Transform ──> PG
Branch File (60 rows)  ─┘        ↓
                        Conflicts: 5 SKU discrepancies logged
```

### CLI usage

```bash
# Run a pipeline with merge
sluice run inventory-sync.pipeline.yaml

# View merge strategies available
sluice merge list-strategies

# Dry-run (validate config, create staging tables, don't load target)
sluice run inventory-sync.pipeline.yaml --dryRun

# Specify incrementalSource for incremental merge (Phase 3)
sluice run inventory-sync.pipeline.yaml --incrementalSource central-db
```

### Error handling

**Config validation (ConfigError):**
- Missing `key` column in any source → fail at config load
- Duplicate source `id` → fail at schema validation
- Invalid strategy name → fail at schema validation

**Runtime (PipelineError):**
- Source table missing → fail during merge phase
- Key column NULL in any source → logged as unmatched (behavior depends on `onUnmatched`)
- SQL builder error → fail with StagingError

**Conflict reporting (informational):**
- Conflicts are logged but don't fail the pipeline
- Use `conflictLog` to export for manual review

---

**Phase 2 merge implementation by: Michael Scott, Caracal Lynx Limited (SC826823)**
  [built-in]  notNull, unique, pattern, email, ukPostcode, maxLength, min, max, allowedValues
  [npm]       ukVatNumber, ukNiNumber, ukSortCode       (@caracal-lynx/etl-rules-uk)
  [file]      ifsCostCentre                             (clients/acme-corp/plugins/)
  [composite] style-coStyleNo, style-coColourCode, positivePrice  (shared/rules.yaml)

Loaded transform plugins (5):
  [built-in]  string, number, decimal, boolean, date, lookup, concat, constant, expression
  [npm]       ukPhoneFormat                             (@caracal-lynx/etl-rules-uk)
  [file]      erpAccountFormat, seasonFromDate          (clients/acme-corp/plugins/)
```
...
9. **Example plugins** — implement the four example plugins above as real files in
   `clients/acme-corp/plugins/` and `clients/style-co/plugins/`. All tests pass.
