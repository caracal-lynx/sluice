...
function reformatIfDate(colName: string, value: unknown, outputFormat: string): string | null {
  ...
  if (!isDateColumn(colName)) return stringifyValue(value);
  const parsed = dayjs(stringifyValue(value));
  if (!parsed.isValid()) return stringifyValue(value);
...
export class BlueCherryTargetAdapter implements TargetAdapter {
  ...
  async connect(config: TargetConfig): Promise<void> {
    // Validation only — but the TargetAdapter contract is async and callers
    // (and tests) consume failures via `.rejects`, so keep this genuinely
    // async rather than throwing synchronously.
    ...
    if (REQUIRED_COLUMNS[config.entity] === undefined) {
