...
const OdooPivotSchema = z.object({
  column: z
    .string()
    .describe(
      'Source column carrying "Key: value" entries. Continuation rows are detected as rows where every column except this one is empty.',
    ),
  keys: z
    .array(z.string())
    .min(1)
  ...
  dropOriginal: z
    ...
    .default(true)
    .describe(
      "When true (default), the pivot column is removed from the output. When false, it carries the parent row's raw value through.",
    ),
});
