...
describe('multi-source schema validation', () => {
  ...
  it('parses a valid multi-source pipeline', () => {
    // Load tests/fixtures/eribe-products-merged.pipeline.yaml
...
Create `tests/fixtures/eribe-products-merged.pipeline.yaml` using the full
example from `docs/phase3-multi-source-merge.md`. This file:
- Must parse cleanly with `ConfigLoader.load()`
- Includes `rename` maps on the `odoo` and `excel` source entries
...
it('sluice check accepts a valid multi-source pipeline', async () => {
  const result = await runCLI(['check', 'tests/fixtures/eribe-products-merged.pipeline.yaml']);
...
it('sluice run rejects a multi-source pipeline with a clear error', async () => {
  const result = await runCLI(['run', 'tests/fixtures/eribe-products-merged.pipeline.yaml']);
...
| `tests/fixtures/eribe-products-merged.pipeline.yaml` | **New fixture.** Full multi-source example |
...
- [ ] `tests/fixtures/eribe-products-merged.pipeline.yaml` exists, parses cleanly, and includes `rename` maps
