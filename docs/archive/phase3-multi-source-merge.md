...
|---|---|---|---|---|
| Eribé Knitwear | Product | SQL Server (master codes, costs) | Odoo ERP (suppliers, lead times) | Excel (descriptions, fibre content) |
...
sources. For example, `STYLE_NO` for Eribé products. Records are matched
...
Best for the Eribé scenario: SQL Server is authoritative for prices, Odoo
...
the output even if Odoo still has it).
...
FULL OUTER JOIN stg_raw_odoo    s2 ON s1.STYLE_NO = s2.STYLE_NO
...
## Full example — Eribé products (SQL Server + Odoo + Excel → BlueCherry)
...
pipeline:
  ...
  description: >
    ...
    Odoo (suppliers), and Excel (descriptions, fibre content).
...
sources:
  ...
  - id: odoo
    priority: 2
    adapter: rest
    endpoint: ${ODOO_BASE}/api/products
    headers:
      Authorization: Bearer ${ODOO_TOKEN}
    pagination:
      type: offset
      ...
      dataField: result
    rename:
      name:         STYLE_DESC    # Odoo field names → canonical source names
...
dq:
  stopOnCritical: true
  rejectionFile: ./output/eribe-products-rejected.csv
  rules:
    - field: STYLE_NO
      sourceId: sql-server
      checks:
...
target:
  adapter: bluecherry
  entity: Style
  output: ./output/eribe-products-bc.csv
  includeHeader: true
