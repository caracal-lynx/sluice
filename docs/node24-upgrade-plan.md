...
| Adapter | Risk | Reason |
|---|---|---|
| `mssql` (Acme Corp) | ⚠️ **HIGH** | `tedious` negotiates TLS with SQL Server. Older SQL Server versions (2012/2014) that haven't been patched may use TLS 1.0/1.1 or short keys. |
...
**Action required:** Run the pre-flight TLS connectivity test (Phase 0, Step 1) against Acme Corp's SQL Server **before any code changes**. If the connection fails, escalate to resolve SQL Server TLS configuration before proceeding.
...
// TODO: Remove when Acme Corp SQL Server is confirmed on TLS 1.2+ natively.
```
...
### Step 1 — Acme Corp SQL Server TLS Connectivity Test (Critical)
...
**If it fails:** Do not proceed with the upgrade until Acme Corp's SQL Server is confirmed to support TLS 1.2 or higher. Apply the temporary `cryptoCredentialsDetails` workaround in Phase 3 as an interim measure.
...
options: {
  ...
  cryptoCredentialsDetails: {
    minVersion: 'TLSv1.2',
    // TODO 2026-05: Remove when Acme Corp SQL Server confirmed TLS 1.2+ native.
...
|---|---|---|---|
| Acme Corp SQL Server TLS 1.0/1.1 | Medium | High — MSSQL adapter fails | Phase 0 pre-flight; temporary `cryptoCredentialsDetails` workaround if needed |
...
If a blocker is encountered (most likely: Acme Corp TLS issue that cannot be resolved quickly):
