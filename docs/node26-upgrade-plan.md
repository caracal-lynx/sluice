...
**Sluice impact:**
- `mssql` adapter (Acme Corp): the tedious driver negotiates TLS with SQL Server. If the SQL Server instance is older (2012/2014) or not patched for TLS 1.2, connections **will fail** on Node 26.
...
**Action required:** ⚠️ **Before upgrading production**, verify the SQL Server version and TLS configuration at Acme Corp. See Section 4, Phase 0.
...
1. **Check SQL Server TLS at Acme Corp:**
   ```sql
   -- Run on the Acme Corp MSSQL instance
   SELECT @@VERSION;
   -- Also check:
...
2. **If SQL Server is old (2012/2014) or version unknown:**
   ...
   ```
   Document this clearly with a `// TODO: remove when Acme Corp SQL Server is confirmed 2022+` comment.
...
|---|---|---|---|
| Acme Corp SQL Server TLS incompatibility | Medium (version unknown) | High — breaks MSSQL adapter | Verify SQL Server version before upgrade; patch if needed |
