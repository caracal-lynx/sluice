...
describe("loadPlugins", () => {
  ...
  it("registered rule plugin is callable and returns null for valid input", async () => {
    ...
    const result = plugin.validate(
      "HELLO",
      { type: "testIsUppercase" as never, severity: "warning" },
      0,
      "FIELD",
    );
  ...
  it("registered transform plugin is callable", async () => {
    ...
    const result = plugin.apply(
      "21",
      {},
      { to: "x", type: "custom", customOp: "testDouble", optional: false },
    );
  ...
  it("throws ConfigError for a malformed merge plugin file (no mergeStrategy export)", async () => {
    ...
    try {
      ...
      await expect(loadPlugins(tmp, rules, transforms, MergeStrategyRegistry)).rejects.toThrow(
        ConfigError,
      );
    } finally {
      rmSync(tmp, { recursive: true, force: true });
