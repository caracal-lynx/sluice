...
function trigger(req: EventEmitter, meta: Record<string, unknown>, rows: Record<string, unknown>[]): void {
...
describe('MssqlSourceAdapter', () => {
  ...
  it('throws SourceError without connection', async () => {
    const adapter = new MssqlSourceAdapter();
    await expect(adapter.connect({ adapter: 'mssql', delimiter: ',', encoding: 'utf-8' } as SourceConfig))
