...
describe("PipelineRunner protected phase methods", () => {
  ...
  it("runDQ delegates to the DQ engine with the provided table name", async () => {
    ...
    const dqSpy = vi
      .spyOn((runner as unknown as { dqEngine: { run: () => Promise<DQSummary> } }).dqEngine, "run")
  ...
  it("runTransform delegates to the transform engine with custom table names", async () => {
    ...
    const transformSpy = vi
      .spyOn(
        (runner as unknown as { transformEngine: { run: () => Promise<TransformResult> } })
          .transformEngine,
        "run",
      )
...
describe("PipelineRunner.run() orchestration", () => {
  ...
  it("passes pluginDirs overrides to loadAllPlugins", async () => {
    ...
    const loadPluginsSpy = vi
      .spyOn(
        runner as unknown as {
          loadAllPlugins: (cwd: string, pluginDirs?: string[]) => Promise<void>;
        },
        "loadAllPlugins",
      )
    ...
    expect(loadPluginsSpy).toHaveBeenCalledWith(expect.any(String), ["./extra-plugins"]);
  });
