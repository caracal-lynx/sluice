...
function pipeline(fields: FieldMapping[], overrides: Partial<Pipeline> = {}): Pipeline {
  return {
...
describe("TransformEngine", () => {
  ...
  it("type: lookup miss with optional: true → null", async () => {
    ...
    try {
      ...
      await engine.run(config, store);
      const out = await store.query<{ VendorNo: string | null }>("SELECT * FROM stg_transformed");
  ...
  it("unmapped: true emits the default placeholder for every row, ignoring from/type", async () => {
    await seed([{ X: "1" }, { X: "2" }]);
    const config = pipeline([{ to: "Division", type: "string", unmapped: true, optional: false }]);
    await engine.run(config, store);
