...
`,
  ...
  it("sluice check path: throws ConfigError for invalid YAML path", async () => {
    await expect(ConfigLoader.load(join(workDir, "nope.yaml"))).rejects.toBeInstanceOf(ConfigError);
  });
  ...
  it("--output flag equivalent: overrides run.outputDir", async () => {
    ...
    expect(result.stateFilePath!.replace(/\\/g, "/")).toContain(overrideDir.replace(/\\/g, "/"));
  });
...
describe("CLI multi-source handling", () => {
  ...
  it("sluice check accepts a valid multi-source pipeline", async () => {
    ...
    const raw = await import("node:fs").then((fs) =>
      fs.readFileSync(multiSourceFixture, "utf-8").replace(/\$\{[^}]+\}/g, "placeholder"),
    );
