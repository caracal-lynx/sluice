...
import { RuleRegistry, TransformRegistry, loadPlugins, loadNpmPlugins } from '../../src/plugins/index.js';
...
describe('sluice plugins command', () => {
  ...
  it('should handle non-existent plugin directory gracefully', async () => {
    ...
    await expect(loadPlugins(nonExistentDir, ruleRegistry, transformRegistry)).resolves.not.toThrow();
  ...
  it('should handle non-existent sluice.config.yaml gracefully', async () => {
    ...
    await expect(loadNpmPlugins(configPath, ruleRegistry, transformRegistry)).resolves.not.toThrow();
  });
  ...
  it('should support applying transform plugins after discovery', async () => {
    ...
    const result = await slugTransform!.apply('Hello World Example', { separator: '-', lowercase: true });
  ...
  it('should support applying hash-value transform after discovery', async () => {
    ...
    const result = await hashTransform!.apply('test@example.com', { algorithm: 'sha256', truncate: 8 });
