 ...
 *
 * Output: docs-site/src/content/docs/reference/pipeline-yaml.md
 *
 * Run via:
...
const OUTPUT_PATH = join(__dirname, '..', 'src', 'content', 'docs', 'reference', 'pipeline-yaml.md');
...
<!--
  ...
  schema.ts to change descriptions, then re-run \`npm run build\` (or
  \`npm run predev\`) to regenerate.
-->
