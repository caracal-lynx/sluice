---
title: How It Works
description: The Sluice runtime architecture — the pipeline phases, DuckDB staging, and the dependency direction between modules.
---
...
The short version: every Sluice pipeline runs through **six core phases plus up to two optional ones**, with an embedded DuckDB instance acting as the staging store between them. The phases are sequenced by `PipelineRunner` (or `MultiSourcePipelineRunner` for pipelines with multiple sources). Adapters are pluggable; everything else is engine code with strict module boundaries.

```
Config load → Extract → [Prep] → [Enrich] → DQ → Transform → [Load] → Run state
```

`Prep` and `Enrich` run only when the pipeline configures them — see [Prep & Enrich Phases](/sluice/reference/prep-and-enrich/). `Load` is skipped under `--dry-run` and `sluice validate`.
...
For a pipeline with a single `source:` block, `PipelineRunner` walks the phases in order:
...
sequenceDiagram
    ...
    participant Enr as Enrich (private)
    ...
    opt prep: configured
        Run->>Prep: apply rules in place on 'stg_raw'
        Prep->>Duck: UPDATE stg_raw SET ...
        Run->>FS: write prep summary JSON
    end

    opt enrich: configured AND not dry-run AND package installed
        Run->>Enr: enrich('stg_raw')
        Enr->>Duck: add + populate writeColumns
    end
...
| 2a. Prep _(optional)_   | `src/prep/`            | Mutate `stg_raw` columns in place — cleanse chains, expressions, lookups              |
| 2b. Enrich _(optional)_ | `src/enrich/` (types)  | Async third-party lookups written into new staging columns; impl is a private package |
...
Prep and Enrich both sit **before** DQ on purpose: they exist to fix or fill in data so that DQ validates the corrected value, not the raw one. They differ in when they are skipped — Enrich does not run under `--dry-run` or `validate`, but Prep does, because DQ's result depends on it.
...
cli.ts → runner.ts / multi-source-runner.ts
            ...
            ├─→ prep/     (Phase 12 — in-place staging fixup)
...
- [Prep & Enrich Phases](/sluice/reference/prep-and-enrich/) — the two optional phases in detail
