...
The `columnOrder` mirrors the IFS import contract. Get one column wrong and IFS rejects the file, so this list is the most important thing to verify before a live run. See [Target Adapters → ifs](/sluice/reference/target-adapters/#ifs--ifs-erp-csv-import).
...
- **Loosening a rule to accommodate dirty data.** If a `pattern` rule rejects every row because the source pads its codes, trim the column in a [`prep:`](/sluice/reference/prep-and-enrich/) rule and keep the rule strict. A rule weakened to fit the data stops being a control.
- **Assuming `enrich:` ran.** Without `@caracal-lynx/sluice-enrich` installed, an `enrich:` block validates fine and is then skipped with a `WARN` — and any DQ rule reading its output columns validates against nothing. Check the log.

## Next steps

- Run it in CI: see [CI/CD Integration](/sluice/guides/ci-cd/).
- Need to consolidate from multiple sources? See [Data Migration Patterns → Pattern 3](/sluice/guides/data-migration-patterns/) for a worked `sources:` + `merge:` example.
- Need to fix data before it is validated? See [Prep & Enrich Phases](/sluice/reference/prep-and-enrich/).
