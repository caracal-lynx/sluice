...
| Key | Type | Required | Default | Description |
|---|---|---|---|---|
| `adapter` | `mssql` \| `pg` \| `csv` \| `xlsx` \| `rest` | yes | — | Source adapter id. One of `mssql`, `pg`, `csv`, `xlsx`, `rest`. |
...
| `file` | string | no | — | Path or glob for file adapters. Required for `csv` and `xlsx`. |
...
| `delimiter` | string | no | `","` | Field delimiter for the `csv` adapter. |
| `encoding` | string | no | `"utf-8"` | File encoding for `csv` and `xlsx` (any Node-supported encoding). |
