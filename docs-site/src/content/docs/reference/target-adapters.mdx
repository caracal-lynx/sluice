...
:::note[There is no `rest` target adapter]
`rest` is a **source** adapter only. `target.adapter: rest` is rejected by `sluice check`. To push transformed rows at an arbitrary HTTP API today, write a `TargetAdapter` plugin; the `bc` adapter is the closest reference implementation.
:::
